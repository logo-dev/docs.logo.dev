...
description: "Display company logos in Google Sheets using Logo.dev's API. Enrich CRM data, company lists, and reports with automatic logo lookup via IMAGE formula. No extensions needed."
keywords:
  [
    "google sheets logo",
...
```javascript By brand name
=IMAGE(CONCAT("https://img.logo.dev/name/", ENCODEURL(A2), "?token=YOUR_API_KEY"))
```
...
<Note>
  When looking up logos by brand name, use `ENCODEURL()` to properly format brand names with spaces or special characters. See the [name lookup documentation](/logo-images/name) for details.
</Note>
