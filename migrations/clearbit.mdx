...
description: "Complete migration guide from Clearbit Logo API to Logo.dev. One-to-one replacement with better features, transparent pricing, and the same team who built Clearbit's logo service."
keywords:
  [
    "clearbit migration",
    "clearbit logo api",
...
<Warning>
  **Clearbit Logo API is shutting down December 8, 2025.** If your app uses
...
<PromptBanner message="Use this pre-built prompt to migrate from Clearbit" prompt="Help me migrate from Clearbit Logo API to Logo.dev. Here's what I need to do:

1. Find and replace all instances of 'https://logo.clearbit.com/' with 'https://img.logo.dev/' in my codebase
2. Sign up for a Logo.dev account at https://www.logo.dev/signup and get my publishable key (starts with pk\_)
3. Add the token parameter to all logo URLs: https://img.logo.dev/stripe.com?token=LOGO_DEV_PUBLISHABLE_KEY
4. Update my attribution link to: <a href='https://logo.dev' title='Logo API'>Logos provided by Logo.dev</a>
...
- **[Name lookup](/logo-images/name)** for brand logos when you only have company names, not domains
