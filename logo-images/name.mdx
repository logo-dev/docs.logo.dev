...
description: "Get brand logos by company name when you don't have a domain. Returns the top Brand Search match with all standard logo size, format, and theme parameters."
sidebarTitle: "Name"
...
<Note>
  The name lookup API is a convenient wrapper over our [Brand Search
  API](/brand-search/introduction) that returns the first result (like "I'm
  Feeling Lucky" for brand logos). It uses the same Brand Search index to find
...
<AccordionGroup>
  ...
  <Accordion title="How accurate is the name matching?">
    ...
    [Brand Search API](/brand-search/introduction) to see all matches and choose
    the right one.
  </Accordion>
  <Accordion title="Can I customize the logo appearance?">
    Yes. All parameters from the [logo endpoint](/logo-images/get) work with
